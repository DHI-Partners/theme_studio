...
solvronix_desk.ThemeStudio = class ThemeStudio {
	...
	_request_initial_state() {
		...
		frappe.call({
			method: "theme_studio.api.get_theme_studio_state",
			callback: function (r) {
				if (
	...
	_load_chart_preview(chartId) {
		...
		frappe.call({
			method: "theme_studio.api.get_chart_preview",
	...
	_call_profile(action, args) {
		var self = this;
		frappe.call({
			method: "theme_studio.api.manage_theme_profile",
	...
	save_draft() {
		...
		var self = this;
		frappe.call({
			method: "theme_studio.api.save_theme_draft",
	...
	import_theme(file) {
		...
		reader.onload = function () {
			try {
				...
				frappe.call({
					method: "theme_studio.api.import_theme_profile",
	...
	show_versions() {
		...
		dialog.fields_dict.versions.$wrapper.on("click", "[data-version]", function () {
			frappe.call({
				method: "theme_studio.api.restore_theme_version",
	...
	_refresh_server_preview() {
		...
		this.preview_timer = setTimeout(function () {
			...
			if (!self.page_active || generation !== self.lifecycle_generation) return;
			frappe.call({
				method: "theme_studio.api.preview_theme_css",
	...
	save() {
		...
		frappe.call({
			method: "theme_studio.api.publish_theme_config",
			args: {
				config: this.config,
